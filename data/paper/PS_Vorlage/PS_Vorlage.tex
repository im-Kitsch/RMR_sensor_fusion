...
Dafür werden zunächst notwendige Grundlagen in Bussystemen und Protokolltheorie vermittelt.
...
Das entstandene Protokoll erfüllt die Anforderungen, die an es gestellt wurden, ist jedoch nicht in der Praxis erprobt.
\end{Zusammenfassung}
\vspace{6pt}

\begin{abstract}
In order to benefit from the greater computing power, this work presents an integration of another microcontroller into an existing system.
Therefore at the beginning the necessary basics about the bus system and protocol theory are introduced. 
A protocol has been designed, with a special focus on the development approach.
...
By using an appropriate test procedure the performance of the protocol is tested while always paying attention to practical feasibility and implementation.
...
Die Steuerung bzw. Regelung des \emph{Firefly} wird durch zwei Mikroprozessoren realisiert, die auf dem sog. \emph{AscTec AutoPilot Board} platziert sind.
...
Zudem besitzt er 2 MByte Flash-Speicher, 512 Kbytes SRAM und 144 Pins, während der Stromverbrauch mit c.a. \SI{250}{mA} zeitgleich sehr gering bleibt.
Das Debugger- und Programmierwerkzeug ST-LinkV2 ist bereits implementiert. Ausreichende Peripheriegeräte sind implementiert, darunter insgesamt vier I2C Schnittstellen sechs SPI Schnittstellen.
...
Das Interface ist ein vollduplexfähiger Bus und weist vergleichsweise hohe Kommunikationsgeschwindigkeiten im Vergleich zu anderen gängigen seriellen Bussen wie USART, I2C auf.
...
Werden in diesem finalen Schritt Designfehler festgestellt, muss wieder zum vorherigen Schritt zurückgekehrt werden. Im Folgenden werden die Ergebnisse der einzelnen durchlaufenen Schritte vorgestellt.
...
Zu den primären Anforderungen gehört zunächst eine ausreichend hohe Transfergeschwindigkeit, ein hohes Maß an Robustheit als auch Echtzeitfähigkeit.
...
Robustheit ist ebenfalls eine wichtige Anforderung, denn sowohl unerkannte Übertragungsfehler als auch ein undefinierter \glqq festgebremster\grqq \space Status führen zum sofortigen Absturz. Diese beiden Fehler gilt es demnach unbedingt zu verhindern.
...
Daraus lässt sich die Mindesttransfergeschwindigkeit ableiten, wenn man zunächst von einer Paketgröße von 150 Byte ausgeht. 
...
Dies wird nun solange fortgeführt, bis keine weitere Null folgt. 
...
Ebenso sei als alternativer Ansatz auf \emph{Two-Way-Handshake-Verfahren} verwiesen (vlg. \cite{protocol_eng}), wobei Nachrichten im Klartext verschickt werden können.
Um trotzdem den Nachrichtenanfang zu ermitteln und eine stabile Verbindung zu überprüfen, synchronisieren sich die Kommunikationsteilnehmer zu Anfang jeder Übertragung durch den vereinbarten \emph{Handshake} (in der Regel durch die Erwiderung eines Antwortbytes zu einem Anfragebyte).
...
Wenn das Paket inhaltlich fertiggestellt ist, was nach Anforderungsspezifikation im \SI{1}{kHz}-Takt geschieht, wird von der Anwendungsschicht in diese Schicht übergegangen. In ihr wird die Checksum generiert und an die (N=4)-PDU angehängt.
...
Hierbei handelt es sich bei den gelesenen Paketen um Sensordaten, die nach der Ausführung von \lstinline{unpack_message()} zur Verfügung stehen.
...
Bei der Fehlererkennung ergibt sich dabei nach \cite{protocol_eng} das Problem, dass Fehler zwar nachgewiesen werden können, jedoch nicht das vollständige Nichtvorhandensein von Fehlern.
...
Die Kombination aus \SI{4}{MHz} SPI Geschwindigkeit und einer Paketrate von \SI{250}{Hz} bietet die höchste Erfolgsübertragungsquote.
...
Während der Implementierung wurden mehrere Probleme wie \emph{Padding} aufgedeckt, die unbedingt zu berücksichtigen sind.
...
Das Protokoll erfüllt einen Großteil der gestellten Anforderungen, ist jedoch definitiv noch optimierbar und erweiterbar.
...
\bibitem{TUT_HLP}
\emph{Tutorial: Coding, Programming and Debugging HLP}, 2018, (URL: \url{https://github.com/im-Kitsch/RMR_sensor_fusion/blob/master/data/tutorials/Tutorial_HLP.pdf}, Zugriff: 22.06.18).
\bibitem{git}
...
\bibitem{TUT_LIDAR}
\emph{Tutorial: Communication between HLP and Lidar Lite v3 via I2C}, 2018, (URL: \url{https://github.com/im-Kitsch/RMR_sensor_fusion/blob/master/data/tutorials/Lidar\%20Tutorial.md}, Zugriff: 03.07.18).
\bibitem{PIC_WIKI}
