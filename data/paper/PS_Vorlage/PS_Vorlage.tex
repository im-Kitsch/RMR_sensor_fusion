...
Die Größe des \emph{Firefly} beträgt 60.5 x 66.5 x 16.5 cm und durch sechs 100 W Motoren kann es bis auf 12 m/s beschleunigt werden.
...
Die Aufgabe \emph{Anpassen der PDU-Größen} wird im entwickelten Protokoll nicht unterstützt, da nur eine einzelne PDU-Definition existiert.
...
Text über den Aufbau den PDU. [GRAFIK MIT AUFBAU DER NACHRICHT]
...
\item Nachrichtenrate (in Anzahl zu sendender Nachrichten pro Sekunde) und
...
\begin{figure}
    \centering
    \includegraphics[width=1.1\linewidth]{pics/test_graphs.pdf}
    \caption{}
    \label{fig:test_graphs}
\end{figure}

ToDo: INTERPRETATION -> PARAMETERWAHL

Ein weiterer Test wurde durchgeführt, um speziell die Robustheit des Protokolls zu testen. Ob korrekt auf Fehlübertragungen reagiert wird, wird bereits im vorherigen Test untersucht. Darüber hinaus muss nach \cite{protocol} jedoch noch auf sogenannte Deadlock- und Livelock-Freiheit getestet werden. Ein Deadlock beschreibt dabei einen Zustand, der sich nie wieder ändern wird. Das Programm wird demnach nicht mehr korrekt ausgeführt, was unbedingt vermieden werden muss. Als Livelock wird ein Zustand beschrieben, in dem ein endloser Zyklus ausgeführt wird, der keine produktiven Aktionen beinhaltet. 
Um auf diese Freiheit zu testen, wird ein mehrstündiger regulärer Datenaustausch initiiert. Am Ende des Tests wird geprüft, ob nach wie vor ein funktionierender Datentransfer vorliegt, oder ob sich einer der beiden genannten Zustände eingestellt hat. Ein 5 stündiger Test lief dabei erfolgreich ab.
